...
class PedidosModel:
    ...
    def generar_pedido_por_obra(self, id_obra, usuario=None):
        """
        Genera un pedido por obra, calcula faltantes y registra auditoría.
        """
        import datetime
        ...
        with self.db.transaction(timeout=30, retries=2):
            ...
            if usuario:
                self.db.ejecutar_query(
                    "INSERT INTO auditorias_sistema (usuario, modulo, accion, fecha) VALUES (?, ?, ?, ?)",
                    (usuario, "Pedidos", f"Generó pedido {id_pedido} para obra {id_obra}", fecha_emision)
                )
        return id_pedido

    def recibir_pedido(self, id_pedido, usuario=None):
        """
        Recibe un pedido, actualiza stock, movimientos y auditoría.
        """
        import datetime
        ...
        with self.db.transaction(timeout=30, retries=2):
            ...
            if usuario:
                self.db.ejecutar_query(
                    "INSERT INTO auditorias_sistema (usuario, modulo, accion, fecha) VALUES (?, ?, ?, ?)",
                    (usuario, "Pedidos", f"Recibió pedido {id_pedido}", fecha)
                )
        return True
